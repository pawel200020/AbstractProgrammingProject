...
void Team::MakeShortAction(Team *team) {
    std::cout << "team named: " << _name << " is starting an action\n";
    Attacker *shooter;
    ...
    for (int i = 0; i < stepsNumber; i++) {
        ...
        std::cout << "He is going to pass to " << *receiver << "\nbut opponent: " << *opponent
                  << " may want to interupt this action\n";
        Sleep(1000);
...
int *Team::GetTeamPowerStats() {
    int *result = new int[_players->size()];
    int i = 0;
    for (auto elem = _players->begin(); elem != _players->end(); elem++, i++) {
        result[i] = (*elem)->GetPower();
    }
    return result;
}
